...
const normalizePhone = (value: string) => { let phone = String(value || '').trim().replace(/\s+/g, '').replace(/[^0-9+]/g, ''); if (phone.startsWith('+')) phone = phone.slice(1); if (phone.startsWith('00')) phone = phone.slice(2); if (phone.startsWith('0')) phone = `964${phone.slice(1)}`; if (!phone.startsWith('964')) phone = `964${phone}`; return phone; };
const phoneCandidates = (phone: string) => [phone, `0${phone.slice(3)}`, `+${phone}`];
const normalizeCode = (value: string) => String(value || '').replace(/[٠-٩]/g, (char) => String('٠١٢٣٤٥٦٧٨٩'.indexOf(char))).replace(/\s/g, '');
...
async function verify(requestToken: string, rawCode: string) {
  ...
  if (error) throw new Error(error.message); if (!request) throw new Error('طلب التحقق غير موجود'); if (!request.telegram_user_id || request.status !== 'code_sent') throw new Error('افتح Telegram واضغط «بدء» أولاً');
