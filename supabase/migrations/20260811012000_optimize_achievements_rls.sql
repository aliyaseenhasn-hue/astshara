...
with check (lawyer_id in (select lp.id from public.lawyer_profiles lp join public.profiles p on p.id=lp.profile_id where p.id=(select auth.uid())));
...
using (lawyer_id in (select lp.id from public.lawyer_profiles lp join public.profiles p on p.id=lp.profile_id where p.id=(select auth.uid())))
with check (lawyer_id in (select lp.id from public.lawyer_profiles lp join public.profiles p on p.id=lp.profile_id where p.id=(select auth.uid())));
