...
create unique index if not exists bookings_active_lawyer_scheduled_at_uidx
on public.bookings (lawyer_id, scheduled_at)
where status not in ('ملغي','مسترد');

do $$
begin
  if not exists (select 1 from pg_constraint where conname='bookings_in_progress_requires_started_at') then
    alter table public.bookings add constraint bookings_in_progress_requires_started_at
      check (status <> 'قيد التنفيذ' or started_at is not null);
  end if;
  if not exists (select 1 from pg_constraint where conname='bookings_completed_requires_timestamps') then
    alter table public.bookings add constraint bookings_completed_requires_timestamps
      check (status <> 'مكتمل' or (started_at is not null and completed_at is not null));
  end if;
  if not exists (select 1 from pg_constraint where conname='bookings_cancelled_requires_timestamp') then
    alter table public.bookings add constraint bookings_cancelled_requires_timestamp
