...
begin
  ...
  return query
  select
    coalesce(lp.full_name, lp_profile.full_name, 'محامي')::text,
    lp_profile.phone::text,
    lp.whatsapp::text,
    client_profile.full_name::text,
    client_profile.phone::text,
    coalesce(v_booking.whatsapp_number, client_profile.phone)::text
...
begin
  ...
  if v_booking.lawyer_id = v_profile_id then
    if v_booking.status = 'مؤكد' then
      ...
      raise exception 'لا يمكن تسجيل عدم حضور العميل في هذه الحالة';
    end if;
    if v_now < v_reference_at + interval '10 minutes' then
      raise exception 'يمكن تسجيل عدم حضور العميل بعد مرور 10 دقائق من الموعد أو بدء الاستشارة';
    end if;
    update public.bookings
    set status = 'بانتظار مراجعة عدم الحضور', consultation_status = 'عدم حضور العميل'
    ...
    update public.bookings
    set status = 'بانتظار مراجعة عدم الحضور', consultation_status = 'عدم حضور المحامي'
    where id = p_booking_id
    returning * into v_booking;
  else
