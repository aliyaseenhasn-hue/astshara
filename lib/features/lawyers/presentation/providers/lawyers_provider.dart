...
final bookingClientNameProvider=FutureProvider.family<String?,String>((ref,bookingId) async {final response=await SupabaseConfig.client.rpc('get_booking_client_name',params:{'p_booking_id':bookingId});if(response is List&&response.isNotEmpty){final row=Map<String,dynamic>.from(response.first as Map);final name=row['full_name'] as String?;if(name!=null&&name.trim().isNotEmpty)return name.trim();}return null;});
