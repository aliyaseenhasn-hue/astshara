...
class LawyerDetailsPage extends ConsumerWidget {
                ...
                SingleChildScrollView(
                  child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                    ...
                    Padding(padding: const EdgeInsets.fromLTRB(16, 20, 16, 10), child: Row(children: [
                      _buildStatItem('${lawyer.yearsExperience}+', 'سنة خبرة'),
                      const SizedBox(width: 8),
                      _buildStatItem('${lawyer.rating}★', 'التقييم', color: AppColors.gold),
                      const SizedBox(width: 8),
                    ...
                    Padding(padding: const EdgeInsets.symmetric(horizontal: 16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      const SizedBox(height: 20),
                      const Text('أنواع الاستشارة', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: AppColors.primary)),
                      const SizedBox(height: 12),
                      _buildServicesRow(),
  ...
  Widget _buildHeroSection(BuildContext context, dynamic lawyer) => Container(
    width: double.infinity, padding: const EdgeInsets.fromLTRB(20, 50, 20, 30),
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
      ...
      Container(width: 80, height: 80, decoration: BoxDecoration(color: AppColors.gold.withValues(alpha: 0.2), borderRadius: BorderRadius.circular(40), border: Border.all(color: AppColors.gold.withValues(alpha: 0.4), width: 2), image: lawyer.avatarUrl != null && lawyer.avatarUrl!.isNotEmpty ? DecorationImage(image: NetworkImage(lawyer.avatarUrl!), fit: BoxFit.cover) : null), child: lawyer.avatarUrl == null || lawyer.avatarUrl!.isEmpty ? Center(child: Text((lawyer.fullName ?? 'م')[0], style: const TextStyle(fontSize: 32, fontWeight: FontWeight.bold, color: AppColors.gold))) : null),
      const SizedBox(height: 16),
      Text(lawyer.fullName ?? 'محامي', textAlign: TextAlign.center, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Colors.white)),
      const Text('محامي · عضو نقابة المحامين العراقيين', textAlign: TextAlign.center, style: TextStyle(fontSize: 13, color: Colors.white54)),
      const SizedBox(height: 12),
