...
class _CustomConsultationRequestsPageState extends State<CustomConsultationRequestsPage> {
  ...
  Future<List<Map<String, dynamic>>> _loadRequests() async {
    ...
    final response = await SupabaseConfig.client.from('custom_consultation_requests').select().eq('lawyer_id', profile['id']).order('created_at', ascending: false);
  ...
  Future<void> _changeStatus(String id, String status) async {
    try {
      await SupabaseConfig.client.rpc('change_custom_consultation_request_status', params: {'p_request_id': id, 'p_new_status': status});
      if (!mounted) return;
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تعذر تحديث حالة الطلب')));
    }
  }

  @override
  Widget build(BuildContext context) => Scaffold(
        appBar: AppBar(title: const Text('طلبات الاستشارة المخصصة'), backgroundColor: AppColors.secondary, foregroundColor: Colors.white),
        body: FutureBuilder<List<Map<String, dynamic>>>(
          future: _requestsFuture,
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator());
            if (snapshot.hasError) return const Center(child: Text('تعذر تحميل الطلبات'));
            final requests = snapshot.data ?? [];
            if (requests.isEmpty) return const Center(child: Text('لا توجد طلبات استشارة مخصصة حالياً'));
    ...
    return Card(
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [Expanded(child: Text(request['subject']?.toString() ?? 'طلب استشارة', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: AppColors.primary))), _StatusChip(status)]),
          ...
          if (createdAt != null) ...[const SizedBox(height: 4), Text('تاريخ الطلب: ${createdAt.day}/${createdAt.month}/${createdAt.year}', style: const TextStyle(fontSize: 12, color: AppColors.textSecondary))],
          if (status == 'جديد') ...[
...
class _StatusChip extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: AppColors.surfaceVariant, borderRadius: BorderRadius.circular(8)), child: Text(status, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold)));
}
