...
class LawyerDashboardPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: CustomScrollView(slivers: [
        SliverAppBar(
          pinned: true, expandedHeight: 245, backgroundColor: AppColors.primary,
          foregroundColor: Colors.white, elevation: 0, automaticallyImplyLeading: false,
          title: const Text('لوحة المحامي', style: TextStyle(fontWeight: FontWeight.w700)), centerTitle: true,
          flexibleSpace: FlexibleSpaceBar(background: Stack(fit: StackFit.expand, children: [
            ...
            SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(20, 72, 20, 20), child: Column(children: [
              ...
              const SizedBox(height: 10), Text(user?.fullName?.trim().isNotEmpty == true ? user!.fullName! : 'أستاذ قانون', style: const TextStyle(color: Colors.white, fontSize: 21, fontWeight: FontWeight.w700)),
              const SizedBox(height: 3), const Text('إدارة الاستشارات والمواعيد من مكان واحد', style: TextStyle(color: Color(0xFFDDE5ED), fontSize: 12)),
            ]))),
        ...
        SliverToBoxAdapter(child: bookingsAsync.when(
          ...
          data: (bookings) => Padding(padding: const EdgeInsets.fromLTRB(AppSizes.p20, 20, AppSizes.p20, 110), child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            _buildOverview(bookings), const SizedBox(height: 28), _sectionHeader('طلبات الاستشارة الواردة', 'عرض الكل', () => context.push('/bookings')), const SizedBox(height: 12),
  ...
  Future<_SetupState> _loadSetupState(WidgetRef ref) async {
    ...
    } catch (_) { return const _SetupState(false, false); }
  }
  ...
  Widget _buildOverview(List<Booking> bookings) {
    ...
    return Container(width: double.infinity, padding: const EdgeInsets.all(24), decoration: BoxDecoration(color: AppColors.primary, borderRadius: BorderRadius.circular(24), boxShadow: [BoxShadow(color: AppColors.primary.withValues(alpha: .06), blurRadius: 24, offset: const Offset(0, 8))]), child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [const Text('ملخص الأداء', style: TextStyle(color: AppColors.goldLight, fontSize: 16, fontWeight: FontWeight.w700)), const SizedBox(height: 18), Row(children: [_metric('$completed', 'استشارات مكتملة', Icons.gavel_rounded), _metric('$active', 'طلبات نشطة', Icons.timelapse_rounded), _metric('${(earnings / 1000).toStringAsFixed(1)} ألف', 'أرباح د.ع', Icons.payments_rounded)])]));
  }
...
class _ProfessionalProfileCard extends StatelessWidget { final VoidCallback onTap; const _ProfessionalProfileCard({required this.onTap}); @override Widget build(BuildContext context) => Card(color: AppColors.surface, elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20), side: const BorderSide(color: AppColors.divider)), child: InkWell(borderRadius: BorderRadius.circular(20), onTap: onTap, child: Padding(padding: const EdgeInsets.all(18), child: Row(textDirection: TextDirection.rtl, children: [Container(width: 48, height: 48, decoration: const BoxDecoration(color: AppColors.goldLight, shape: BoxShape.circle), child: const Icon(Icons.badge_outlined, color: AppColors.primary)), const SizedBox(width: 12), const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text('ملفي المهني', style: TextStyle(color: AppColors.primary, fontSize: 16, fontWeight: FontWeight.w900)), SizedBox(height: 5), Text('أضف نبذتك المهنية وارفع صور القرارات والإنجازات والشهادات التي تريد عرضها لطالبي الاستشارة.', textAlign: TextAlign.right, style: TextStyle(color: AppColors.textSecondary, fontSize: 12, height: 1.5))])), const SizedBox(width: 8), const Icon(Icons.arrow_back_ios_new_rounded, size: 17, color: AppColors.textSecondary)])))); }
class _SetupState { final bool hasPackage; final bool hasSlots; const _SetupState(this.hasPackage, this.hasSlots); bool get complete => hasPackage && hasSlots; }
class _SetupAlert extends StatelessWidget { final _SetupState setup; final VoidCallback onTap; const _SetupAlert({required this.setup, required this.onTap}); @override Widget build(BuildContext context) { final title = !setup.hasPackage ? 'أكمل معلومات استقبال الاستشارات' : 'حدد أوقات التوفر لاستقبال الطلبات'; final message = !setup.hasPackage ? 'أضف باقات الاستشارات وأسعارها أولاً، ثم انتقل إلى أوقات التوفر.' : 'تم إعداد الباقات. بقي تحديد مواعيد التوفر حتى يتمكن طالب الاستشارة من الحجز.'; return Card(color: AppColors.secondary, elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)), child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [Row(textDirection: TextDirection.rtl, children: [Container(width: 42, height: 42, decoration: const BoxDecoration(color: AppColors.gold, shape: BoxShape.circle), child: const Icon(Icons.assignment_turned_in_rounded, color: AppColors.secondary)), const SizedBox(width: 12), Expanded(child: Text(title, textAlign: TextAlign.right, style: const TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w900)))]), const SizedBox(height: 10), Text(message, textAlign: TextAlign.right, style: const TextStyle(color: Colors.white70, height: 1.5, fontSize: 12)), const SizedBox(height: 14), FilledButton.icon(onPressed: onTap, icon: const Icon(Icons.arrow_back_rounded), label: Text(!setup.hasPackage ? 'إكمال المعلومات' : 'تعديل الباقات وإضافة الأوقات'), style: FilledButton.styleFrom(backgroundColor: AppColors.gold, foregroundColor: AppColors.secondary, padding: const EdgeInsets.symmetric(vertical: 13), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))))]))); } }
class _BookingCard extends ConsumerWidget { final Booking booking; const _BookingCard({required this.booking}); @override Widget build(BuildContext context, WidgetRef ref) { final scheme = Theme.of(context).colorScheme; final clientNameAsync = ref.watch(bookingClientNameProvider(booking.id)); final clientName = clientNameAsync.valueOrNull; final displayName = clientName != null && clientName.trim().isNotEmpty ? clientName.trim() : 'اسم العميل غير متوفر'; return Card(margin: const EdgeInsets.only(bottom: 12), color: scheme.surfaceContainerLowest, child: InkWell(borderRadius: BorderRadius.circular(16), onTap: () => context.push('/booking-details', extra: booking), child: Padding(padding: const EdgeInsets.all(20), child: Row(textDirection: TextDirection.rtl, children: [CircleAvatar(radius: 24, backgroundColor: AppColors.goldLight.withValues(alpha: .45), child: const Icon(Icons.person_outline, color: AppColors.goldDark)), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Row(textDirection: TextDirection.rtl, children: [Expanded(child: Text(displayName, textAlign: TextAlign.right, style: TextStyle(fontWeight: FontWeight.w700, color: scheme.onSurface))), const SizedBox(width: 8), _StatusChip(status: booking.status)]), const SizedBox(height: 6), Text(booking.consultationType ?? 'استشارة قانونية', textAlign: TextAlign.right, style: TextStyle(color: scheme.onSurfaceVariant, fontSize: 13)), const SizedBox(height: 5), Text(_formatDate(booking.scheduledAt), textAlign: TextAlign.right, style: TextStyle(color: scheme.onSurfaceVariant, fontSize: 11))])), const SizedBox(width: 6), Icon(Icons.chevron_left_rounded, color: scheme.onSurfaceVariant)])))); }
...
class _StatusChip extends StatelessWidget { final String status; const _StatusChip({required this.status}); @override Widget build(BuildContext context) { final normalized = status.trim(); final accepted = normalized == 'مؤكد' || normalized == 'مكتمل' || normalized == 'قيد التنفيذ'; final cancelled = normalized.contains('إلغاء') || normalized.contains('رفض') || normalized.contains('عدم حضور'); final background = accepted ? AppColors.acceptedBg : (cancelled ? AppColors.cancelledBg : AppColors.pendingBg); final foreground = accepted ? AppColors.acceptedText : (cancelled ? AppColors.cancelledText : AppColors.pendingText); return Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 5), decoration: BoxDecoration(color: background, borderRadius: BorderRadius.circular(999), border: Border.all(color: Theme.of(context).colorScheme.outlineVariant.withValues(alpha: .45))), child: Text(normalized, style: TextStyle(color: foreground, fontSize: 9.5, fontWeight: FontWeight.w700))); } }
class _NotificationBell extends StatelessWidget { final int unreadCount; final VoidCallback onTap; const _NotificationBell({required this.unreadCount, required this.onTap}); @override Widget build(BuildContext context) => Stack(children: [IconButton(onPressed: onTap, icon: const Icon(Icons.notifications_none_rounded, color: Colors.white)), if (unreadCount > 0) Positioned(right: 8, top: 7, child: Container(width: 16, height: 16, decoration: const BoxDecoration(color: AppColors.gold, shape: BoxShape.circle), alignment: Alignment.center, child: Text(unreadCount > 9 ? '9+' : '$unreadCount', style: const TextStyle(color: AppColors.primary, fontSize: 8, fontWeight: FontWeight.w900))) ]); }
