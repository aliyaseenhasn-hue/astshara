...
class _ChatPageState extends ConsumerState<ChatPage> {
  ...
  void _send() {
    if (_messageController.text.trim().isNotEmpty) {
      ref.read(chatControllerProvider.notifier).send(widget.conversationId, _messageController.text.trim());
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        titleSpacing: 0,
        title: Row(children: [
          CircleAvatar(radius: 18, backgroundColor: scheme.surfaceContainerHighest, child: const Icon(Icons.person, size: 20, color: AppColors.primary)),
          const SizedBox(width: 10),
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(otherPartyNameAsync.maybeWhen(data: (name) => name ?? 'محادثة', orElse: () => '...'), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            const Row(children: [CircleAvatar(radius: 3, backgroundColor: Colors.green), SizedBox(width: 4), Text('متصل الآن', style: TextStyle(fontSize: 11))]),
          ]),
        ]),
        actions: [
    ...
    return Container(
      ...
      child: SafeArea(child: Row(children: [
        Expanded(child: Container(
          ...
          child: Row(children: [
            IconButton(onPressed: () {}, icon: Icon(Icons.attach_file, color: scheme.onSurfaceVariant)),
            Expanded(child: TextField(controller: _messageController, onChanged: (v) => setState(() {}), maxLines: null, decoration: const InputDecoration(hintText: 'اكتب رسالتك هنا...', border: InputBorder.none, contentPadding: EdgeInsets.symmetric(horizontal: 8)))),
          ]),
        )),
        const SizedBox(width: 12),
        CircleAvatar(backgroundColor: AppColors.gold, radius: 22, child: IconButton(onPressed: _send, icon: Icon(_messageController.text.isEmpty ? Icons.mic : Icons.send, color: AppColors.textOnPrimary, size: 22))),
      ])),
    );
  }
}
