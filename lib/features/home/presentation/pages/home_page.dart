...
class HomePage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: SafeArea(child: ListView(padding: const EdgeInsets.fromLTRB(16, 18, 16, 100), children: [
        Row(children: [Expanded(child: Text('استشارة', style: TextStyle(color: scheme.onSurface, fontSize: 28, fontWeight: FontWeight.w900))), IconButton(onPressed: () => context.push('/notifications'), icon: const Icon(Icons.notifications_none_rounded))]),
        const SizedBox(height: 24),
        Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: scheme.primaryContainer.withValues(alpha: .42), borderRadius: BorderRadius.circular(24)), child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text('استشر محامياً متخصصاً', textAlign: TextAlign.right, style: TextStyle(color: scheme.onSurface, fontSize: 22, fontWeight: FontWeight.w900)), const SizedBox(height: 8), Text('اختر المجال القانوني المناسب واحجز موعد استشارة بسهولة.', textAlign: TextAlign.right, style: TextStyle(color: scheme.onSurfaceVariant)), const SizedBox(height: 16), FilledButton.icon(onPressed: () => context.push('/legal-categories'), icon: const Icon(Icons.arrow_back_rounded), label: const Text('اختر التخصص'))])),
