...
final adminUsersProvider = FutureProvider.family.autoDispose<List<Map<String, dynamic>>, ({String search, String role, String status})>((ref, f) async {
  final rows = await Supabase.instance.client.rpc('admin_list_users', params: {'p_search': f.search.isEmpty ? null : f.search, 'p_role': f.role.isEmpty ? null : f.role, 'p_status': f.status.isEmpty ? null : f.status});
...
class _AdminUsersPageState extends ConsumerState<AdminUsersPage> {
  final search = TextEditingController(); String role = ''; String status = ''; String query = '';
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(title: const Text('إدارة المستخدمين'), actions: [IconButton(onPressed: refresh, icon: const Icon(Icons.refresh))]), body: Column(children: [
      ...
      Expanded(child: users.when(loading: () => const Center(child: CircularProgressIndicator()), error: (e, _) => Center(child: Padding(padding: const EdgeInsets.all(24), child: Text('تعذر تحميل المستخدمين: $e', textAlign: TextAlign.center))), data: (items) => items.isEmpty ? const Center(child: Text('لا يوجد مستخدمون')) : RefreshIndicator(onRefresh: () async { ref.invalidate(adminUsersProvider(filter)); }, child: ListView.separated(itemCount: items.length, separatorBuilder: (_, __) => const Divider(height: 1), itemBuilder: (_, i) => _userTile(items[i])))))
    ]));
  }
  Widget _userTile(Map<String, dynamic> u) { final name = (u['full_name']?.toString().trim().isNotEmpty ?? false) ? u['full_name'].toString().trim() : 'بدون اسم'; final role = u['role']?.toString() ?? ''; final status = u['status']?.toString() ?? ''; return ListTile(leading: CircleAvatar(child: Text(name.substring(0, 1))), title: Text(name), subtitle: Text('${_roleLabel(role)} • ${u['phone'] ?? u['email'] ?? 'لا توجد بيانات اتصال'}'), trailing: Chip(label: Text(_statusLabel(status)))); }
