...
class _CancellationRequestsPageState extends State<CancellationRequestsPage> {
  ...
  Future<void> _review(Map<String, dynamic> request) async {
    ...
    if (status != 'بانتظار مراجعة الإدارة') return;
    final decision = await showModalBottomSheet<String>(
      context: context,
      isScrollControlled: true,
      builder: (context) => _DecisionSheet(request: request),
    );
    if (decision == null || !mounted) return;
    try {
      double? rate;
      if (decision == 'الموافقة مع غرامة') {
        rate = await showDialog<double>(context: context, builder: (_) => _PenaltyRateDialog());
        if (rate == null) return;
      }
      ...
      if (decision == 'الموافقة مع غرامة' && rate != null) {
        ...
        final confirmed = await showDialog<bool>(
          ...
          builder: (context) => AlertDialog(
            ...
            content: Text('قيمة الاستشارة: ${bookingPrice.toStringAsFixed(0)} د.ع\nنسبة الغرامة: ${rate.toStringAsFixed(0)}%\nقيمة الغرامة: ${amount.toStringAsFixed(0)} د.ع\nالمستفيد من التعويض: ${request['client_name']}\n\nسيتم الحساب والاعتماد نهائياً داخل قاعدة البيانات.'),
            actions: [TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('رجوع')), FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('اعتماد القرار'))],
          ),
        );
      ...
      body: FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, index) => _RequestCard(request: requests[index], onReview: _review),
          );
        },
      ),
    );
    ...
    return Card(
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          _row('الحجز الداخلي', request['booking_id']),
          _row('نوع الاستشارة', request['consultation_type']),
          ...
          if (request['description']?.toString().trim().isNotEmpty == true) _row('تفاصيل الاستشارة', request['description']),
          if (pending) ...[
            const SizedBox(height: 14),
            FilledButton.icon(onPressed: () => onReview(request), icon: const Icon(Icons.rule_outlined), label: const Text('مراجعة طلب الإلغاء')),
          ],
        ]),
      ),
    );
    ...
    return SafeArea(
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          FilledButton(onPressed: () => Navigator.pop(context, 'الموافقة بدون غرامة'), child: const Text('الموافقة بدون غرامة')),
          const SizedBox(height: 8),
          FilledButton.tonal(onPressed: () => Navigator.pop(context, 'الموافقة مع غرامة'), child: const Text('الموافقة مع غرامة')),
          const SizedBox(height: 8),
...
class _PenaltyRateDialogState extends State<_PenaltyRateDialog> {
  ...
  @override
  Widget build(BuildContext context) {
    final priceText = 'اختر نسبة الغرامة، وسيتم حساب المبلغ النهائي خادمياً.';
    return AlertDialog(
      title: const Text('نسبة الغرامة'),
      content: Column(mainAxisSize: MainAxisSize.min, children: [
        Text(priceText, textAlign: TextAlign.right),
        const SizedBox(height: 10),
        DropdownButtonFormField<double>(
          value: _rate,
          decoration: const InputDecoration(labelText: 'نسبة الغرامة'),
          items: [for (int i = 5; i <= 100; i += 5) DropdownMenuItem(value: i.toDouble(), child: Text('$i%'))],
          onChanged: (value) => setState(() => _rate = value ?? 20),
        ),
      ]),
      actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('إلغاء')), FilledButton(onPressed: () => Navigator.pop(context, _rate), child: const Text('متابعة'))],
    );
  }
}
