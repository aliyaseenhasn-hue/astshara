...
class _FinancialManagementPageState extends State<FinancialManagementPage> {
    ...
    try {
      ...
      payouts = List<Map<String, dynamic>>.from(
        await SupabaseConfig.client
            ...
            .select('id,lawyer_id,amount,currency,status,wallet_number,created_at')
  ...
  Future<void> _status(Map<String, dynamic> row, String status) async {
    try {
      await SupabaseConfig.client.rpc(
        ...
        params: {'p_payout_id': row['id'], 'p_status': status},
      );
      await _load();
    } catch (e) {
      if (mounted) {
  ...
  String statusLabel(String status) =>
      {'pending_review': 'بانتظار المراجعة', 'approved': 'تمت الموافقة', 'processing': 'قيد التنفيذ', 'paid': 'تم التحويل', 'rejected': 'مرفوض', 'failed': 'فشل'}[status] ?? status;
                  ...
                  ...payouts.map(
                    (row) => Card(
                      child: ListTile(
                        ...
                        subtitle: Text(
                          '${statusLabel('${row['status']}')} • ${row['wallet_number'] ?? ''}',
                        ),
