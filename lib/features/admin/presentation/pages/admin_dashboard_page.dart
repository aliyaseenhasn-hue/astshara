...
class AdminDashboardPage extends ConsumerWidget {
      ...
      body: statsAsync.when(
        ...
        data: (stats) => SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: AppSizes.p16),
            _buildAdminActionTile('مراقبة الدفعات', '${stats['pending_payments']} دفعات قيد التحقق الآلي', Icons.monitor_heart_outlined, AppColors.primary, () => context.push('/admin/payments')),
            _buildAdminActionTile('طلبات تغيير التخصص', 'مراجعة التخصصات وهوية النقابة', Icons.badge_outlined, AppColors.secondary, () => context.push('/admin/specialization-change-requests')),
            _buildAdminActionTile('طلبات انضمام المحامين', '${stats['pending_verifications']} طلبات مراجعة', Icons.person_add, Colors.amber, () => context.push('/admin/lawyer-verifications')),
  ...
  Widget _buildAdminActionTile(String title, String subtitle, IconData icon, Color color, VoidCallback onTap) => Card(
    ...
    child: ListTile(leading: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: color.withValues(alpha: .1), borderRadius: BorderRadius.circular(8)), child: Icon(icon, color: color, size: 24)), title: Text(title, style: const TextStyle(fontWeight: FontWeight.bold)), subtitle: Text(subtitle, style: const TextStyle(fontSize: 12)), trailing: const Icon(Icons.arrow_forward_ios, size: 14), onTap: onTap),
  );
}
