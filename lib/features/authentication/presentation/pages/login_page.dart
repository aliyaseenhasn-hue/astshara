...
class _LoginPageState extends ConsumerState<LoginPage> {
  ...
  String _errorMessage(Object error) {
    if (_isNetworkError(error)) return 'تعذر الاتصال بالإنترنت. تحقق من اتصالك بالإنترنت ثم حاول تسجيل الدخول مرة أخرى.';
      ...
      try {
        ...
        if (status == 'telegram_verified' && !verifying && !closed) {
          ...
          try {
            ...
            poller?.cancel(); closed = true;
            if (mounted && Navigator.of(context).canPop()) Navigator.of(context).pop();
          } catch (e) {
            if (mounted) setDialogState(() { verifying = false; message = _errorMessage(e); });
          }
        ...
        if (_isNetworkError(e) && mounted && !closed) setDialogState(() { message = 'تعذر الاتصال بالإنترنت. تحقق من اتصالك بالإنترنت ثم حاول مرة أخرى.'; });
      }
    }
        ...
        builder: (dialogContext) => StatefulBuilder(
          builder: (dialogContext, setDialogState) {
            ...
            return AlertDialog(
              ...
              content: Directionality(textDirection: TextDirection.rtl, child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                const Icon(Icons.telegram, size: 48), const SizedBox(height: 12), Text(message, textAlign: TextAlign.right),
                if (!showCode) ...[const SizedBox(height: 16), const LinearProgressIndicator()],
                if (showCode) ...[const SizedBox(height: 16), TextField(controller: codeController, keyboardType: TextInputType.number, textAlign: TextAlign.center, textDirection: TextDirection.ltr, maxLength: 6, autofocus: true, decoration: const InputDecoration(labelText: 'رمز التحقق', hintText: '000000', counterText: '', prefixIcon: Icon(Icons.lock_outline_rounded)), onChanged: (value) { final normalized = _normalizeDigits(value); if (normalized != value) codeController.value = codeController.value.copyWith(text: normalized, selection: TextSelection.collapsed(offset: normalized.length)); })],
              ])),
              actions: [
                TextButton(onPressed: verifying ? null : () { closed = true; poller?.cancel(); Navigator.of(dialogContext).pop(); }, child: const Text('إلغاء')),
                if (showCode) FilledButton.icon(icon: verifying ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.verified_rounded), label: Text(verifying ? 'جارٍ التحقق...' : 'تحقق ودخول'), onPressed: verifying ? null : () async {
                  ...
                  try { await _finishTelegramLogin(requestToken); poller?.cancel(); closed = true; if (dialogContext.mounted) Navigator.of(dialogContext).pop(); }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Center(child: SingleChildScrollView(padding: const EdgeInsets.all(24), child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 520), child: Directionality(textDirection: TextDirection.rtl, child: Form(key: _formKey, child: Card(elevation: 0, child: Padding(padding: const EdgeInsets.all(24), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        Text('أدخل رقم هاتفك العراقي لإتمام الدخول بأمان عبر Telegram.', textAlign: TextAlign.center, style: TextStyle(color: scheme.onSurfaceVariant)), const SizedBox(height: 24),
