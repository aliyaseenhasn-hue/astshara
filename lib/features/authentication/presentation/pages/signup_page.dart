        ...
        builder: (dialogContext) => StatefulBuilder(
          builder: (context, setDialogState) => AlertDialog(
            ...
            content: Column(
              ...
              children: const [
                Text('تم فتح Telegram. اضغط «بدء» إذا ظهرت، وسيصلك رمز تحقق من 6 أرقام داخل محادثة بوت استشارة.', textAlign: TextAlign.right),
                SizedBox(height: 16),
                Text('أدخل الرمز هنا بعد وصوله.', textAlign: TextAlign.right),
              ],
            ),
            actions: [
              ...
              child: ConstrainedBox(
                ...
                child: LayoutBuilder(
                  builder: (context, constraints) {
                    ...
                    return wide
                        ? Row(children: [const Expanded(child: _SignupBrand()), const SizedBox(width: 44), SizedBox(width: 480, child: form)])
                        : form;
                  },
                ),
              ),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: const [
        ...
        SizedBox(height: 10),
        Text('ابدأ باستخدام رقم هاتفك عبر Telegram أو حساب Google، ثم أكمل بياناتك المهنية عند الحاجة.', style: TextStyle(fontSize: 15, height: 1.8)),
        SizedBox(height: 28),
        _SignupBenefit(icon: Icons.phone_android_rounded, title: 'رقم الهاتف', text: 'تحقق سريع وآمن من خلال رمز Telegram.'),
        SizedBox(height: 14),
        _SignupBenefit(icon: Icons.account_circle_outlined, title: 'حساب Google', text: 'تسجيل سريع دون الحاجة إلى كلمة مرور جديدة.'),
        SizedBox(height: 14),
        _SignupBenefit(icon: Icons.gavel_rounded, title: 'حساب المحامي', text: 'اختر نوع الحساب وأكمل الملف المهني بعد التسجيل.'),
      ]),
    );
  }
}
...
class _SignupBenefit extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(textDirection: TextDirection.rtl, children: [
      Container(width: 42, height: 42, decoration: BoxDecoration(color: s.surface.withValues(alpha: .7), borderRadius: BorderRadius.circular(13)), child: Icon(icon, color: s.primary)),
      const SizedBox(width: 12),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text(title, style: TextStyle(fontWeight: FontWeight.w900, color: s.onSurface)), Text(text, textAlign: TextAlign.right, style: TextStyle(fontSize: 11, color: s.onSurfaceVariant))])),
    ]);
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return Form(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        Text('إنشاء حساب', textAlign: TextAlign.right, style: TextStyle(fontSize: 30, fontWeight: FontWeight.w900, color: s.onSurface)),
        const SizedBox(height: 7),
        Text('اختر نوع حسابك ثم سجّل باستخدام رقم الهاتف أو Google.', textAlign: TextAlign.right, style: TextStyle(color: s.onSurfaceVariant, height: 1.6, fontSize: 13)),
        ...
        Card(elevation: 0, child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          Text('نوع الحساب', textAlign: TextAlign.right, style: TextStyle(fontWeight: FontWeight.w800, color: s.onSurface)),
          ...
          const SizedBox(height: 18),
          Row(children: [Expanded(child: Divider(color: s.outlineVariant)), Padding(padding: const EdgeInsets.symmetric(horizontal: 12), child: Text('أو', style: TextStyle(color: s.onSurfaceVariant))), Expanded(child: Divider(color: s.outlineVariant))]),
          const SizedBox(height: 18),
          SizedBox(height: 52, child: OutlinedButton.icon(onPressed: loading ? null : onGoogle, icon: const Icon(Icons.account_circle_outlined), label: const Text('المتابعة باستخدام Google', style: TextStyle(fontWeight: FontWeight.w800)))),
        ]))),
        const SizedBox(height: 12),
        TextButton(onPressed: loading ? null : () => context.go('/login'), child: Text('لديك حساب بالفعل؟ تسجيل الدخول', style: TextStyle(color: s.primary, fontWeight: FontWeight.w700))),
        const SizedBox(height: 8),
