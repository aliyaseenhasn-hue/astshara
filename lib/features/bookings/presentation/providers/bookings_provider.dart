...
final bookingDetailsProvider=FutureProvider.family<Map<String,dynamic>?,String>((ref,bookingId) async { final booking=await SupabaseConfig.client.from('bookings').select('user_id,lawyer_id, consultation_type, consultation_mode, manual_payment_required, manual_received_amount, description, document_url, package_name, package_description, package_duration_minutes').eq('id',bookingId).maybeSingle(); if(booking==null)return null; final result=Map<String,dynamic>.from(booking); final client=await SupabaseConfig.client.from('profiles').select('full_name').eq('id',booking['user_id']).maybeSingle(); final lawyer=await SupabaseConfig.client.from('profiles').select('full_name,avatar_url').eq('id',booking['lawyer_id']).maybeSingle(); result['client_name']=client?['full_name']??'غير متوفر'; result['lawyer_name']=lawyer?['full_name']??'غير متوفر'; result['lawyer_avatar_url']=lawyer?['avatar_url']; return result; });
