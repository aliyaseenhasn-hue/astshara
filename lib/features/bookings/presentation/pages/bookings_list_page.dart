...
class BookingsListPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: false,
        actions: [
          IconButton(onPressed: () => ref.invalidate(isLawyer ? lawyerBookingsProvider : userBookingsProvider), icon: const Icon(Icons.refresh_rounded), tooltip: 'تحديث'),
        ],
      ),
            ...
            return Center(
              child: Padding(
                ...
                child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  Container(width: 82, height: 82, decoration: BoxDecoration(color: scheme.surfaceContainerHighest, borderRadius: BorderRadius.circular(24)), child: Icon(Icons.calendar_month_outlined, size: 42, color: scheme.primary)),
                ...
                return Card(
                  ...
                  child: InkWell(
                    ...
                    child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(children: [
                        Container(width: 46, height: 46, decoration: BoxDecoration(color: scheme.primaryContainer, borderRadius: BorderRadius.circular(14)), child: Icon(isLawyer ? Icons.person_outline_rounded : Icons.balance_rounded, color: scheme.primary)),
                      ...
                      Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 11), decoration: BoxDecoration(color: scheme.surfaceContainerHighest, borderRadius: BorderRadius.circular(13)), child: Row(children: [Icon(Icons.schedule_rounded, size: 18, color: scheme.primary), const SizedBox(width: 8), Expanded(child: Text(DateFormat('yyyy/MM/dd - HH:mm').format(booking.scheduledAt), style: TextStyle(fontSize: 12, color: scheme.onSurface))), Text('${booking.price} د.ع', style: TextStyle(fontWeight: FontWeight.bold, color: scheme.onSurface))])),
  ...
  Widget _action(BuildContext context, IconData icon, String label, VoidCallback onPressed, {bool outlined = false, bool dark = false}) {
    final scheme = Theme.of(context).colorScheme;
    final style = outlined ? OutlinedButton.styleFrom(minimumSize: const Size.fromHeight(46)) : ElevatedButton.styleFrom(minimumSize: const Size.fromHeight(46), backgroundColor: dark ? scheme.primary : scheme.primary, foregroundColor: scheme.onPrimary);
    return SizedBox(width: double.infinity, child: outlined ? OutlinedButton.icon(onPressed: onPressed, icon: Icon(icon, size: 19), label: Text(label), style: style) : ElevatedButton.icon(onPressed: onPressed, icon: Icon(icon, size: 19), label: Text(label), style: style));
  }
}
...
class _StatusBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 6), decoration: BoxDecoration(color: scheme.primaryContainer, borderRadius: BorderRadius.circular(20)), child: Text(label, style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: scheme.onPrimaryContainer)));
  }
}
