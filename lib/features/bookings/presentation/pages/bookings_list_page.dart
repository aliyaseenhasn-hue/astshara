...
class BookingsListPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: bookingsAsync.when(
        ...
        error: (_, __) => Center(
          child: Text('تعذر تحميل الاستشارات', style: TextStyle(color: scheme.onSurface)),
        ),
        data: (bookings) {
          if (bookings.isEmpty) {
            return Center(
                ...
                child: Column(
                  ...
                  children: [
                    Container(
                      ...
                      child: Icon(Icons.calendar_month_outlined, size: 42, color: scheme.primary),
                    ),
                            ...
                            Row(
                              children: [
                                Container(
                                  ...
                                  child: Icon(
                                    isLawyer ? Icons.person_outline_rounded : Icons.balance_rounded,
                            ...
                            Container(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  Expanded(
                                    child: Text(
                                      DateFormat('yyyy/MM/dd - HH:mm').format(booking.scheduledAt),
  ...
  Widget _buildActions(BuildContext context, WidgetRef ref, dynamic booking, bool isLawyer) {
    // Online payment is independent of lawyer approval. A successful payment
    // moves the booking to "قيد مراجعة المحامي"; approval happens afterwards.
    ...
    // In-office consultations have no online payment. The lawyer records the
    // amount received, after which the booking can proceed normally.
  ...
  Widget _action(
    ...
    final style = outlined
        ...
        : ElevatedButton.styleFrom(
            ...
            backgroundColor: dark ? scheme.primary : scheme.primary,
            foregroundColor: scheme.onPrimary,
          );
    return SizedBox(
      width: double.infinity,
      child: outlined
          ? OutlinedButton.icon(onPressed: onPressed, icon: Icon(icon, size: 19), label: Text(label), style: style)
          : ElevatedButton.icon(onPressed: onPressed, icon: Icon(icon, size: 19), label: Text(label), style: style),
    );
  }
}
