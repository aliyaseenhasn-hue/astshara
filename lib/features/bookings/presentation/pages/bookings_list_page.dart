...
class BookingsListPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final isLawyer = ref.watch(currentUserRoleProvider).maybeWhen(data: (r) => r == 'lawyer', orElse: () => false);
                ...
                return Card(
                  ...
                  child: InkWell(
                    ...
                    child: Padding(padding: const EdgeInsets.all(16), child: Row(children: [
                      if (!isLawyer) ...[CircleAvatar(radius: 27, backgroundColor: scheme.surfaceContainerHighest, backgroundImage: lawyerAvatar != null && lawyerAvatar.isNotEmpty ? NetworkImage(lawyerAvatar) : null, child: lawyerAvatar == null || lawyerAvatar.isEmpty ? Icon(Icons.person_rounded, color: scheme.onSurfaceVariant) : null), const SizedBox(width: 12)],
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text(displayName, textAlign: TextAlign.right, style: TextStyle(fontWeight: FontWeight.w800, color: scheme.onSurface)), const SizedBox(height: 6), Text(booking.serviceType.isNotEmpty ? booking.serviceType : 'استشارة قانونية', style: TextStyle(color: scheme.onSurfaceVariant, fontSize: 12)), const SizedBox(height: 8), Text(booking.status, style: TextStyle(color: scheme.primary, fontWeight: FontWeight.w700, fontSize: 12))])),
                      const SizedBox(width: 8), Icon(Icons.chevron_left_rounded, color: scheme.onSurfaceVariant),
                    ])),
                  ),
                );
              });
