...
class _CreateBookingPageState extends ConsumerState<CreateBookingPage> {
  ...
  Future<void> _loadSlots() async {
    ...
    try {
      final response = await SupabaseConfig.client
          ...
          .lte('starts_at', DateTime.now().add(const Duration(days: 30)).toUtc().toIso8601String())
          .order('starts_at');
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('تعذر تحميل المواعيد: $e')));
    } finally {
  ...
  Future<void> _submit() async {
    ...
    // Preserve the existing controller contract while sending both values to the secure RPC.
  ...
  Widget _buildTypeStep() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      const SizedBox(height: 12),
      TextFormField(controller: _descriptionController, maxLines: 4, decoration: InputDecoration(labelText: 'وصف مختصر للموضوع', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))),
      const SizedBox(height: 12),
      TextFormField(controller: _whatsappController, keyboardType: TextInputType.phone, decoration: InputDecoration(labelText: 'رقم واتسابك للتواصل', hintText: '077XXXXXXXX', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)))),
      const SizedBox(height: 12),
      ...
      _NavigationButtons(onBack: () => setState(() => _step = 0), onNext: _loadSlots, nextLabel: _loadingSlots ? 'جاري تحميل المواعيد...' : 'اختيار الموعد'),
    ]);
  }
  ...
  Widget _buildReviewStep(dynamic user) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      const SizedBox(height: 16),
      _NavigationButtons(onBack: () => setState(() => _step = 2), onNext: _submit, nextLabel: 'الدفع وتأكيد الحجز'),
    ]);
  }
}
