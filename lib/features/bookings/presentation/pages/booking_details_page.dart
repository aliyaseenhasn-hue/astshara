    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          _buildSection(title: 'معلومات العميل', icon: Icons.person_outline, child: Text(clientNameAsync.maybeWhen(data: (name) => name ?? 'عميل الاستشارة', orElse: () => 'جاري التحميل...'), style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold))),
          const SizedBox(height: 16),
          _buildSection(title: 'نوع الاستشارة والموعد', icon: Icons.calendar_today_outlined, child: extraDetailsAsync.when(data: (details) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            _buildDetailRow('الباقة:', details?['package_name'] ?? 'غير محددة'), const SizedBox(height: 8),
            _buildDetailRow('النوع:', _getConsultationTypeText(details?['consultation_type'])), const SizedBox(height: 8),
            _buildDetailRow('التاريخ:', DateFormat('yyyy-MM-dd').format(booking.scheduledAt)), const SizedBox(height: 8),
            _buildDetailRow('الوقت:', DateFormat('HH:mm').format(booking.scheduledAt)), const SizedBox(height: 8),
            _buildDetailRow('المدة:', '${details?['package_duration_minutes'] ?? 30} دقيقة'), const SizedBox(height: 8),
            _buildDetailRow('السعر:', '${booking.price} د.ع'),
          ]), loading: () => const LinearProgressIndicator(), error: (_, __) => const Text('تعذر تحميل تفاصيل الموعد'))),
          const SizedBox(height: 16),
          _buildSection(title: 'وصف الموضوع', icon: Icons.description_outlined, child: extraDetailsAsync.when(data: (details) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(details?['description'] ?? 'لا يوجد وصف متاح لهذا الطلب.'),
            if (details?['document_url'] != null) ...[
              const SizedBox(height: 16), const Divider(), const SizedBox(height: 8), const Text('المستندات المرفقة:', style: TextStyle(fontWeight: FontWeight.bold)), const SizedBox(height: 8),
