    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          if (isLawyer) ...[
            _section(context, 'بيانات العميل', Icons.person_outline_rounded, clientName.when(
              data: (name) => Column(children: [_row(context, 'اسم العميل', name?.trim().isNotEmpty == true ? name!.trim() : (booking.userName?.trim().isNotEmpty == true ? booking.userName! : 'غير متوفر')), _row(context, 'نوع الحساب', 'طالب خدمة')]),
              loading: () => const LinearProgressIndicator(),
              error: (_, __) => _row(context, 'اسم العميل', booking.userName?.trim().isNotEmpty == true ? booking.userName! : 'غير متوفر'),
            )),
          ...
          if (_canReportNoShow(isLawyer)) OutlinedButton.icon(onPressed: () => _reportNoShow(context, ref, isLawyer), icon: const Icon(Icons.report_problem_outlined), label: Text(isLawyer ? 'الإبلاغ عن عدم حضور العميل' : 'الإبلاغ عن عدم حضور المحامي')),
