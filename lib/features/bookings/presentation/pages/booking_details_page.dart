...
class BookingDetailsPage extends ConsumerWidget {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          _buildSection(
            title: 'معلومات العميل', icon: Icons.person_outline,
            child: Text(clientNameAsync.maybeWhen(data: (name) => name ?? 'عميل الاستشارة', orElse: () => 'جاري التحميل...'), style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          ),
          const SizedBox(height: 16),
          _buildSection(
            title: 'نوع الاستشارة والموعد', icon: Icons.calendar_today_outlined,
            child: extraDetailsAsync.when(
              data: (details) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          const SizedBox(height: 16),
          _buildSection(
            title: 'وصف الموضوع', icon: Icons.description_outlined,
            child: extraDetailsAsync.when(
              data: (details) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                if (details?['document_url'] != null) ...[
                  ...
                  const SizedBox(height: 8),
                  OutlinedButton.icon(onPressed: () async { await launchUrl(Uri.parse(details!['document_url'])); }, icon: const Icon(Icons.file_present, size: 18), label: const Text('فتح المستند المرفق')),
                ],
              ]),
          ...
          const SizedBox(height: 16),
          _buildSection(
            title: 'حالة الدفع', icon: Icons.receipt_long_outlined,
            child: paymentAsync.when(
              data: (payment) => payment == null
                  ? const Text('بانتظار إكمال الدفع.', style: TextStyle(color: AppColors.error))
          ...
          if (booking.status == 'بانتظار التأكيد')
            Row(children: [
              Expanded(child: ElevatedButton(onPressed: () => _updateStatus(context, ref, 'مؤكد'), style: ElevatedButton.styleFrom(backgroundColor: AppColors.success, foregroundColor: Colors.white), child: const Text('موافقة وتأكيد'))),
          ...
          if (booking.status == 'مؤكد')
            ElevatedButton.icon(onPressed: () => _updateStatus(context, ref, 'قيد التنفيذ'), icon: const Icon(Icons.play_arrow), label: const Text('بدء الاستشارة'), style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 16))),
          if (booking.status == 'قيد التنفيذ')
            ElevatedButton.icon(onPressed: () => _updateStatus(context, ref, 'مكتمل'), icon: const Icon(Icons.check_circle_outline), label: const Text('إنهاء الاستشارة'), style: ElevatedButton.styleFrom(backgroundColor: AppColors.success, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 16))),
  ...
  Widget _buildStatusHeader() {
    ...
    switch (booking.status) {
      ...
      case 'ملغي': color = AppColors.error; text = 'الحجز ملغي'; break;
      case 'مسترد': color = Colors.grey; text = 'تم استرداد المبلغ'; break;
    }
