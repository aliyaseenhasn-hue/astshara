...
class BookingDetailsPage extends ConsumerWidget {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          _section(context, 'بيانات الحجز', Icons.calendar_month_outlined, detailsAsync.when(data: (d) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            if (isOwner) _row(context, 'اسم المحامي', d?['lawyer_name']?.toString() ?? 'غير متوفر'),
            _row(context, 'الباقة', d?['package_name']?.toString() ?? 'استشارة مختلفة'),
            _row(context, 'نوع الاستشارة', d?['consultation_type']?.toString() ?? 'غير محددة'),
            _row(context, 'طريقة التنفيذ', _consultationMethod(d?['consultation_type']?.toString(), d?['consultation_mode']?.toString())),
          ...
          ]), loading: () => const Center(child: CircularProgressIndicator()), error: (_, __) => const Text('تعذر تحميل تفاصيل الحجز'))),
          const SizedBox(height: 12),
          _section(context, 'وصف الموضوع', Icons.description_outlined, detailsAsync.when(data: (d) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(d?['description']?.toString() ?? 'لا يوجد وصف متاح.', style: TextStyle(color: scheme.onSurface, height: 1.5)), if (d?['document_url'] != null) Align(alignment: Alignment.centerRight, child: OutlinedButton.icon(onPressed: () => launchUrl(Uri.parse(d!['document_url'] as String)), icon: const Icon(Icons.file_present_outlined), label: const Text('فتح المستند المرفق')))],), loading: () => const Text('جاري التحميل...'), error: (_, __) => const Text('تعذر تحميل الوصف'))),
          const SizedBox(height: 12),
          ...
          if (['مؤكد', 'قيد التنفيذ', 'مكتمل'].contains(booking.status)) ...[
            const SizedBox(height: 12),
            _section(context, 'معلومات التواصل', Icons.contact_phone_outlined, contactAsync.when(data: (c) => c == null ? const Text('لا توجد معلومات تواصل متاحة.') : _contactContent(context, c, isLawyer), loading: () => const Center(child: CircularProgressIndicator()), error: (e, _) => Text(e.toString().replaceFirst('Exception: ', '')))),
          ],
          ...
          if (canReview) ...[
            _infoCard(context, booking.status == 'قيد مراجعة المحامي' ? 'تم الدفع لهذا الحجز وأصبح بانتظار قرارك. عند الرفض ينتقل إلى بانتظار الاسترداد.' : 'هذا الطلب بانتظار مراجعتك. يمكنك الموافقة أو رفض الطلب قبل بدء الاستشارة.', Icons.rule_rounded),
            const SizedBox(height: 10),
            Row(children: [Expanded(child: ElevatedButton.icon(onPressed: () => _reviewBooking(context, ref, true), icon: const Icon(Icons.check_circle_outline), label: const Text('الموافقة'), style: _button(context))), const SizedBox(width: 10), Expanded(child: OutlinedButton.icon(onPressed: () => _reviewBooking(context, ref, false), icon: const Icon(Icons.cancel_outlined), label: const Text('رفض الطلب'), style: OutlinedButton.styleFrom(foregroundColor: scheme.error, padding: const EdgeInsets.symmetric(vertical: 16))))]),
          ],
          if (isOwner && booking.status == 'قيد انتظار الدفع') ElevatedButton.icon(onPressed: () => context.push('/upload-payment', extra: booking), icon: const Icon(Icons.payment_rounded), label: const Text('إكمال الدفع'), style: _button(context)),
          if (isLawyer && booking.status == 'مؤكد') _startButton(context, ref, detailsAsync),
          if (isLawyer && booking.status == 'قيد التنفيذ') ElevatedButton.icon(onPressed: () => _updateStatus(context, ref, 'مكتمل'), icon: const Icon(Icons.check_circle_outline), label: const Text('إنهاء الاستشارة'), style: _button(context, color: scheme.primary)),
          if (canReportNoShow) OutlinedButton.icon(onPressed: () => _reportNoShow(context, ref, isLawyer), icon: const Icon(Icons.report_problem_outlined), label: Text(isLawyer ? 'الإبلاغ عن عدم حضور العميل' : 'الإبلاغ عن عدم حضور المحامي'), style: OutlinedButton.styleFrom(foregroundColor: scheme.error, padding: const EdgeInsets.symmetric(vertical: 14))),
          if (isOwner && booking.status == 'مكتمل') ElevatedButton.icon(onPressed: () => showDialog(context: context, builder: (_) => ReviewDialog(bookingId: booking.id, lawyerId: booking.lawyerId)), icon: const Icon(Icons.star_outline_rounded), label: const Text('تقييم الاستشارة'), style: _button(context)),
        ]),
      ),
    );
  }
  ...
  Future<void> _reviewBooking(BuildContext context, WidgetRef ref, bool approved) async { try { await ref.read(bookingsRepositoryProvider).reviewBooking(booking.id, approved); ref.invalidate(lawyerBookingsProvider); ref.invalidate(userBookingsProvider); ref.invalidate(bookingPaymentProvider(booking.id)); if (!context.mounted) return; ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(approved ? 'تمت الموافقة على طلب الحجز' : 'تم رفض الطلب وتحويله إلى بانتظار الاسترداد إذا كان مدفوعاً'))); Navigator.pop(context); } catch (e) { if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.toString().replaceFirst('Exception: ', '')), backgroundColor: schemeError(context))); } }
  Color schemeError(BuildContext context) => Theme.of(context).colorScheme.error;
  Future<void> _updateStatus(BuildContext context, WidgetRef ref, String status) async { try { await ref.read(bookingsRepositoryProvider).updateBookingStatus(booking.id, status); ref.invalidate(lawyerBookingsProvider); ref.invalidate(userBookingsProvider); if (context.mounted) Navigator.pop(context); } catch (e) { if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.toString().replaceFirst('Exception: ', '')), backgroundColor: Theme.of(context).colorScheme.error)); } }
  Future<void> _reportNoShow(BuildContext context, WidgetRef ref, bool isLawyer) async { try { await ref.read(bookingsRepositoryProvider).reportNoShow(booking.id, isLawyer: isLawyer); ref.invalidate(lawyerBookingsProvider); ref.invalidate(userBookingsProvider); if (context.mounted) Navigator.pop(context); } catch (e) { if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.toString().replaceFirst('Exception: ', '')), backgroundColor: Theme.of(context).colorScheme.error)); } }
}
