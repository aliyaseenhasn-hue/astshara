...
class BookingDetailsPage extends ConsumerWidget {
  ...
  Future<void> _reportNoShow(BuildContext context, WidgetRef ref, bool isLawyer) async { try { await ref.read(bookingsRepositoryProvider).reportNoShow(booking.id, isLawyer); ref.invalidate(lawyerBookingsProvider); ref.invalidate(userBookingsProvider); if (context.mounted) Navigator.pop(context); } catch (e) { if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.toString().replaceFirst('Exception: ', '')), backgroundColor: Theme.of(context).colorScheme.error)); } }
}
