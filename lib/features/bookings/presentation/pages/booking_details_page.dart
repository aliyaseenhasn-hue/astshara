...
class BookingDetailsPage extends ConsumerWidget {
          ...
          children: [
            ...
            _section(
              ...
              detailsAsync.when(
                ...
                loading: () => const CircularProgressIndicator(),
                error: (_, __) => const Text('تعذر تحميل تفاصيل الحجز'),
              ),
            ),
                ...
                data: (d) => Column(
                  ...
                  children: [
                    ...
                    if (d?['document_url'] != null)
                      OutlinedButton.icon(
                        onPressed: () => launchUrl(Uri.parse(d!['document_url'] as String)),
            ...
            if (isOwner && booking.status == 'قيد مراجعة المحامي')
              _section(
                ...
                const Text('تم استلام الدفع بنجاح، والحجز الآن بانتظار موافقة المحامي. لن تظهر معلومات التواصل ولن تبدأ الاستشارة حتى يوافق المحامي.', style: TextStyle(height: 1.6)),
              ),
            if (isOwner && booking.status == 'بانتظار الاسترداد')
              _section(
                ...
                const Text('رفض المحامي الطلب بعد إتمام الدفع. تم تسجيل الطلب بانتظار استرداد المبلغ، ولن يتم بدء الاستشارة.', style: TextStyle(height: 1.6)),
              ),
                            ...
                            if (isLawyer) ...[
                              ...
                              if (c['client_whatsapp'] != null && booking.status == 'قيد التنفيذ')
                                ElevatedButton.icon(
                                  ...
                                  style: _button(context, color: const Color(0xFF25D366)),
                                ),
                            ],
                            if (!isLawyer) ...[
                              Text('المحامي: ${c['lawyer_name'] ?? 'غير متوفر'}'),
            ...
            if (isOwner && booking.status == 'مؤكد')
              _sessionInfo(context, 'الحجز مؤكد. ستبدأ الاستشارة في الموعد المحدد، ويمكنك فتح واتساب بعد بدء الجلسة.', Icons.schedule),
            if (isLawyer && booking.status == 'قيد التنفيذ')
              _sessionInfo(context, 'الاستشارة قيد التنفيذ. بعد التواصل مع العميل أنهِ الجلسة من زر إنهاء الاستشارة.', Icons.play_circle_outline),
            if (booking.status == 'بانتظار مراجعة عدم الحضور')
              _sessionInfo(context, 'تم تسجيل عدم الحضور لهذه الاستشارة. الطلب الآن بانتظار مراجعة الإدارة قبل اتخاذ قرار بشأن الاسترداد.', Icons.report_problem_outlined),
            ...
            if (isOwner && booking.status == 'قيد انتظار الدفع')
              ElevatedButton.icon(onPressed: () => context.push('/upload-payment', extra: booking), icon: const Icon(Icons.payment), label: const Text('إكمال الدفع'), style: _button(context)),
            if (isLawyer && booking.status == 'مؤكد') _startButton(context, ref, detailsAsync),
            if (isLawyer && booking.status == 'قيد التنفيذ')
              ElevatedButton.icon(onPressed: () => _updateStatus(context, ref, 'مكتمل'), icon: const Icon(Icons.check_circle_outline), label: const Text('إنهاء الاستشارة'), style: _button(context, color: AppColors.success)),
            if (canReportNoShow)
              OutlinedButton.icon(onPressed: () => _reportNoShow(context, ref, isLawyer), icon: const Icon(Icons.report_problem_outlined), label: Text(isLawyer ? 'الإبلاغ عن عدم حضور العميل' : 'الإبلاغ عن عدم حضور المحامي'), style: OutlinedButton.styleFrom(foregroundColor: AppColors.error, padding: const EdgeInsets.symmetric(vertical: 14))),
            if (isOwner && booking.status == 'مكتمل')
              ElevatedButton.icon(onPressed: () => showDialog(context: context, builder: (_) => ReviewDialog(bookingId: booking.id, lawyerId: booking.lawyerId)), icon: const Icon(Icons.star), label: const Text('تقييم الاستشارة'), style: _button(context)),
          ],
        ),
      ),
    );
  ...
  Widget _startButton(BuildContext context, WidgetRef ref, AsyncValue<Map<String, dynamic>?> detailsAsync) {
    ...
    if (now.isAfter(closesAt)) {
      return Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: AppColors.error.withValues(alpha: .08), borderRadius: BorderRadius.circular(12)), child: const Text('انتهى وقت بدء الاستشارة لهذا الموعد. يمكنك الإبلاغ عن عدم حضور العميل إذا لم تبدأ الجلسة.', textAlign: TextAlign.center));
    }
    if (!canStart) {
      final minutes = opensAt.difference(now).inMinutes + 1;
      return Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: AppColors.primary.withValues(alpha: .08), borderRadius: BorderRadius.circular(12)), child: Text('يمكن بدء الاستشارة قبل الموعد بـ 5 دقائق. المتاح بعد حوالي $minutes دقيقة.', textAlign: TextAlign.center));
    }
  ...
  Widget _row(BuildContext context, String label, String value) {
    final scheme = Theme.of(context).colorScheme;
    return Padding(padding: const EdgeInsets.only(bottom: 8), child: Row(children: [Text('$label: ', style: TextStyle(color: scheme.onSurfaceVariant)), Expanded(child: Text(value, style: TextStyle(fontWeight: FontWeight.w600, color: scheme.onSurface)))]));
  }
  ...
  String _paymentMethod(String value) => switch (value) {
    'ZainCash' || 'زين كاش' => 'زين كاش',
    'Qi Card' || 'Qicard' || 'كي كارد' => 'كي كارد',
    'MasterCard' || 'mastercard' => 'ماستركارد',
    'bank_transfer' || 'تحويل مصرفي' => 'تحويل مصرفي',
    _ => value,
  };
  ...
  Future<void> _reportNoShow(BuildContext context, WidgetRef ref, bool isLawyer) async {
    final confirmed = await showDialog<bool>(context: context, builder: (dialogContext) => AlertDialog(title: const Text('تأكيد الإبلاغ'), content: Text(isLawyer ? 'سيتم تسجيل أن العميل لم يحضر، وسيبقى الحجز بانتظار مراجعة الإدارة. هل تريد المتابعة؟' : 'سيتم تسجيل أن المحامي لم يحضر، وسيبقى الحجز بانتظار مراجعة الإدارة. هل تريد المتابعة؟'), actions: [TextButton(onPressed: () => Navigator.pop(dialogContext, false), child: const Text('إلغاء')), ElevatedButton(onPressed: () => Navigator.pop(dialogContext, true), child: const Text('تأكيد'))]));
  ...
  Future<void> _updateStatus(BuildContext context, WidgetRef ref, String status) async {
    try {
      ...
      if (status == 'قيد التنفيذ') {
        if (booking.consultationMode == 'في المكتب') {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('بدأت الاستشارة في المكتب بنجاح.')));
          Navigator.pop(context);
          return;
        }
        ...
        try {
          final response = await SupabaseConfig.client.rpc('get_booking_participant_contact_info', params: {'p_booking_id': booking.id});
          if (!context.mounted) return;
