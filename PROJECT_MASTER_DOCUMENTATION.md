...
- [x] إصلاح أخطاء التخطيط (Overflow) في واجهة الويب. ✅
- [x] تحسين نظام فرز وفلترة المحامين حسب التقييم والتوفر. ✅
- [x] مراجعة الإنتاج (Production Readiness Review) وتحسين UX/UI بالكامل. ✅
- [x] إضافة ميزة طلب تغيير التخصص للمحامي مع رقابة إدارية. ✅
- [x] تفعيل كافة صفحات الإعدادات (طرق الدفع، مركز المساعدة، إلخ). ✅
- [ ] إعدادات اللغة (قيد التنفيذ).

---

## 8. Technical Fixes Log (سجل التعديلات التقنية)
### تحديث أغسطس 2026: مراجعة الجاهزية للإنتاج وتطوير UX/UI
1. **نظام تصميم موحد (Design System)**: اعتماد لوحة ألوان `Royal Navy` و `Deep Elegant Gold` مع خلفيات `Off White` لضمان تباين عالٍ وفخامة بصرية.
2. **فصل التجارب**: عزل واجهة المحامي تماماً عن العميل عبر `LawyerDashboardPage` ومنع التفاعل الذاتي (حجز/تقييم النفس).
3. **تطوير الإعدادات**: تفعيل صفحات طرق الدفع (إدارة رقم المحفظة)، مركز المساعدة (اتصال مباشر)، وإعدادات التطبيق.
4. **تعديل البيانات الشخصية**: تمكين المستخدم من تحديث اسمه وصورته الشخصية مباشرة من صفحة الحساب.
5. **إدارة التخصصات**: إضافة ميزة اختيار تخصصات متعددة مع نظام "طلب تغيير التخصص" الذي يخضع لموافقة المسؤولين لضمان المصداقية.
6. **تحسين الأداء**: استخدام `Slivers` و `CustomScrollView` لضمان استجابة وسلاسة عالية على الويب والأجهزة المحمولة، مع معالجة كافة تحذيرات الـ Analyzer.
### تحديث أغسطس 2026: إصلاح خطأ 400 وتحسين واجهة إكمال الملف
1. **حل مشكلة 400 Bad Request**: تم إزالة حقل `full_name` من عملية `upsert` لجدول `lawyer_profiles` لمنع تعارض المخطط (Schema Mismatch)، مع الاعتماد على الربط (Join) مع جدول `profiles` لجلب الاسم.
2. **تحسين سجلات الأخطاء (Logging)**: إضافة معالجة متقدمة للاستثناءات `PostgrestException` في الـ Repository لإظهار تفاصيل الخطأ الدقيقة من Supabase في وحدة التحكم.
3. **تطوير واجهة المستخدم (UX)**: إضافة زر إغلاق (X) في صفحة إكمال الملف الشخصي يسمح للمستخدم بإلغاء العملية وتسجيل الخروج بعد تأكيد عبر نافذة حوار (Dialog).
4. **تحديث البيانات (Data Mapping)**: تحديث الـ Model والـ Entity للمحامين ليدعم تخزين وعرض الاسم الكامل القادم من الجداول المرتبطة بشكل صحيح.
5. **مزامنة الكود**: تشغيل `build_runner` لتحديث كافة الملفات المولدة (`.freezed.dart`, `.g.dart`) لضمان استقرار النظام.
...
### تحديث أغسطس 2026: إصلاح أخطاء التخطيط وتحسين تجربة العميل
1. **حل مشكلة Overflow**: تم زيادة الارتفاع الممتد (`expandedHeight`) لـ `SliverAppBar` في قائمة المحامين إلى 200 وتعديل الهوامش لمنع ظهور أخطاء التجاوز (RenderFlex overflow) عند عرض النصوص الطويلة في الهيدر.
2. **تحسين نظام الفرز والبحث**:
    - تحديث `lawyersListProvider` ليقوم بفرز المحامين تلقائياً بحيث يظهر المحامي المتاح (Available) أولاً، ثم الأعلى تقييماً (Rating)، ثم الأكثر مراجعات.
    - جعل البحث عن التخصصات أكثر مرونة عبر تنظيف النصوص وتجاهل حالة الأحرف.
3. **تعزيز استقرار البيانات**:
    - تحديث `LawyersRepositoryImpl` لإضافة نظام معالجة أخطاء متقدم مع "خطة بديلة" (Fallback) تضمن جلب البيانات الأساسية حتى في حال فشل الربط المعقد للجداول.
    - تصحيح أسماء الحقول في واجهة العميل لتتوافق مع نموذج البيانات الجديد (`specializations`).
4. **إصلاح واجهة التطبيق**: نقل طبقة التحميل العالمية إلى الـ `builder` الخاص بـ `MaterialApp` لحل مشكلة فقدان اتجاه النص (`No Directionality widget found`) التي كانت تسبب انهيار التطبيق عند بدء التشغيل.
