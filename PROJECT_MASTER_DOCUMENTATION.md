...
- [x] إصلاح معالجة التخصصات (String vs Array mismatch). ✅
- [x] توحيد المعرفات وتأمين عمليات الـ Upsert للبروفايل. ✅
- [x] تحسين واجهة البروفايل (توسيط الصورة ودعم الأفاتار). ✅
- [x] تحديث لوحة تحكم المحامي (إضافة أزرار الخروج وحذف الحساب). ✅
- [x] إزالة خيارات التعديل غير الضرورية للمحامي. ✅
- [x] إصلاح عرض المحامين في قائمة العميل (تصحيح أسماء الحقول). ✅
- [ ] إعدادات اللغة (قيد التنفيذ).

---

## 8. Technical Fix Log (سجل التعديلات التقنية)
### تحديث أغسطس 2026: إصلاح شامل لنظام العرض والبيانات
1. **إصلاح خطأ "المحامون لا يظهرون"**: تم اكتشاف تضارب في أسماء الحقول بين الـ Entity (`specializations` - قائمة) والواجهة (`specialization` - نص)، مما كان يسبب فشلاً في بناء قائمة المحامين. تم تصحيح الواجهة لتستخدم القائمة مع دمجها بنص مفصول بفواصل.
2. **تحسين الهوية البصرية للمحامي**: دمج صورة الأفاتار (`avatar_url`) في بطاقات المحامين وفي صفحة التفاصيل، مع إضافة دعم للصور الدائرية المتمركزة.
3. **تحديث الـ Repository**: إضافة حقل `avatar_url` لاستعلامات الجلب لضمان ظهور الصور في كافة أجزاء التطبيق.
4. **تزامن الكود المولد**: إعادة تشغيل `build_runner` لضمان أن كافة التعديلات في الـ Entities قد تم عكسها في الملفات المولدة (`.freezed.dart`).
### تحديث أغسطس 2026: إصلاح شامل لنظام الدخول والتوثيق
1. **حل مشكلة تضارب أنواع البيانات (Specialization)**: تم تحديث `LawyerProfileModel` ليكون مرناً في قراءة التخصصات من قاعدة البيانات سواء كانت نصاً بسيطاً أو مصفوفة Postgres. كما تم تحديث `LawyersRepositoryImpl` لدمج القائمة في نص مفصول بفواصل عند الحفظ لتجنب خطأ 400.
2. **تأمين إنشاء الحسابات**: تحويل عملية تحديث البروفايل في `AuthRepositoryImpl` إلى `upsert` مع فرض توحيد المعرف (`id` و `auth_id`) لضمان عدم فشل العمليات المعتمدة على البروفايل في حال تأخر الـ Trigger.
3. **تحسين مرونة الـ Model**: إضافة محول (Converter) ذكي للـ JSON يتعامل مع كافة تنسيقات المصفوفات القادمة من Supabase.
4. **تحديث سجلات التتبع**: التأكد من نجاح تدفق البيانات من خلال مراقبة سجلات Edge Functions وربطها بالهوية الموحدة للمستخدم.
